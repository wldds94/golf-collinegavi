...
<article class="gara <?php echo esc_attr( (isset($evidence) && true === $evidence) ? 'first' : '' ); ?>" data-giri="<?= $gara->durata ?>">
    <div class="textSection">
        <div class="titleSection">
            <? if($linkedPost): ?>
            <h2 class="link"><a href='<?php echo get_permalink($linkedPost[0]->ID) ?>'><?= esc_html__(($dateObj->format('l'))).' '.' '.$gara->data.' '.$linkedPost[0]->post_title ?></a></h2>
            <? else: ?>
            <h2><?php esc_html__(($dateObj->format('l'))).' '.$gara->data ?> <?= $gara->nome_gara ?></h2>
            <? endif; ?>
        </div>
